...
*Бэк - первый запрос после долгого простоя может занять
на несколько секунд дольше.*

![watchlist](./watchlist.png)

**Стек:** React 19, TypeScript, Redux Toolkit · Node.js, Express 5, Mongoose, JWT, bcryptjs · MongoDB Atlas · Vercel + Railway

## Что реально сделано

- **Auth.** bcrypt cost 12 для пароля. HS256 JWT, 24 часа жизни. Длина
...
так что ключ публичен. Правильное решение - проксировать `/search/movie` и
`/movie/:id/videos` через свой бэк и держать ключ только там. В roadmap.

## Чего не хватает → план

| Проблема | Решение |
|---|---|
| Нет `helmet` и rate limiting на `/auth/*` - credential stuffing не блокируется | `express-rate-limit` + `helmet` |
| Нет composite unique индекса `{ userId, movieId }` - двойной клик в двух вкладках может создать дубли | Добавить индекс + обработка дублей |
| Email не нормализуется (`Vasya@x.com` ≠ `vasya@x.com`) | Нормализация при регистрации/логине |
| Ключ TMDB виден в бандле фронта | Backend-прокси для TMDB |
| Один smoke-тест на фронте, на бэке тестов нет, CI нет | Supertest на auth + IDOR-устойчивость, тесты slice'ов, GitHub Actions |
| Нет retry/backoff на 429 от TMDB | Добавить backoff, показывать понятную ошибку |
